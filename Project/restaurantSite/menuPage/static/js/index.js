...
//Check screen width and convert to mobile view if necessary
document.addEventListener('DOMContentLoaded', function() {
    console.log('Mobile')
    ...
    // Function to handle opening and closing of the cart
    function toggleCart() {
        if (!isCartOpen) {
    ...
    // Close the cart when clicking outside of it
    window.addEventListener('click', function(event) {
        if (!cartTab.contains(event.target) && !openCartButton.contains(event.target)) {
            body.classList.remove('showCart');
            isCartOpen = false;
        }
    });
    ...
    function adjustBehavior() {
        if (window.innerWidth <= 767) {
            ...
            // Add any desktop-specific behavior here
        }
    }

    // Adjust behavior on load and resize
    adjustBehavior();
